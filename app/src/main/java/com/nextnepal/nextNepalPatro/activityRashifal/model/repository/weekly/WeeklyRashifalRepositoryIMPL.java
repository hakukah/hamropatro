...
public class WeeklyRashifalRepositoryIMPL implements WeeklyRashifalRepository {
    ...
    public WeeklyRashifalRepositoryIMPL(Context context, RashifalApiService rashifalApiService) {
    ...
    public Observable<RashifalEntity> getRashifalFromNetwork() {
        disposable = rashifalApiService.getRashifalWeekly()
    ...
    private void storeCache(RashifalEntity rashifalEntity) {
        try {
            SharedPreferences sharedPreferences = context.getSharedPreferences("Cache_rashifal_weekly", Context.MODE_PRIVATE);
            SharedPreferences.Editor editor = sharedPreferences.edit();
            Gson gson = new Gson();
            String json = gson.toJson(rashifalEntity);
            editor.putString("rashifal_weekly", json);
            editor.commit();
        } catch (Exception e) {
        }
    }
    ...
    private RashifalEntity retrieveCache() {
        Gson gson = new Gson();
        SharedPreferences sharedPreferences = context.getSharedPreferences("Cache_rashifal_weekly", Context.MODE_PRIVATE);
        String json = sharedPreferences.getString("rashifal_weekly", "");
    ...
    public boolean isUpToDate() {
        return System.currentTimeMillis() - timeStamp < RASHIFALCONST.STALE_WEEK_RASHIFAL_MS;
    }
}
